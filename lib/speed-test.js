...
function pause() {
  if (!paused) {
    paused = true
    console.log('Speed test paused')
    statusChangeCallbacks.forEach(x => x({ paused: true, activeProcesses }))
  }
}
...
function resume() {
  if (paused) {
    paused = false
    console.log('Speed test resumed')
    statusChangeCallbacks.forEach(x => x({ paused: false, activeProcesses }))
  }
}
...
module.exports.onStatusChange = cb => {
